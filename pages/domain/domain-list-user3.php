                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    ...
                                    foreach ($doms as $name) {
                                        ...
                                        // } else if ($os_platform == "unix") {
                                        //     $os_icon = "<i class='fab fa-uniregistry'></i>";
                                        // } else if ($os_platform == "mac") {
            ...
            <form id="createDomainForm" name="createDomain" role="form" action="">
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-6">
                            <div class="form-group">
                                <input type="text" class="form-control" id="checkOsType" name="os_platform" value="" placeholder="OS Type" readonly>
                            </div>
                        </div>
                    </div>
...
<script>
    // send form data to create user in endpoint localhost://api/users in json
    // function myFunction() {

    //     var x = document.getElementById("createDomainForm");
    //     var txt = "";
    //     var i;
    ...
    // vm<br>newVM<br>windows<br>2<br>on<br>4<br>MiB<br>GiB<br>/var/lib/libvirt/images/Noob.qcow2<br>newVM.qcow2<br>40<br>qcow2<br>default<br>default<br>qcow2<br>default<br>/home/ubuntu/images/Win10pro.iso<br>52:54:00:46:d1:18<br>default<br>default<br>create-domain<br><br>Submit<br>
    var form = document.getElementById('createDomainForm');
    form.addEventListener('submit', function() {
        // e.preventDefault();
        //encode json from php

        // userid must be a dummmy number
        var userid = "1";
        var uuid = '<?php echo $_SESSION['username']; ?>';
        var action = '<?php echo $_SESSION['action']; ?>';
        var username = '<?php echo $_SESSION['username']; ?>';
        var instance_type = document.getElementById("instance_type").value;
        var domain_name = document.getElementById("domain_name").value;
        var os = document.getElementById("checkOsType").value;
        var vcpu = document.getElementById("vcpu").value;
        var cores = document.getElementById("tcores").value;
        var threads = document.getElementById("threads").value;
        var memory = document.getElementById("memory").value;
        var memory_unit = document.querySelector('input[name="memory_unit"]:checked').value;
        var source_file_volume = document.querySelector('input[name="source_file_volume"]:checked').value;
        var volume_image_name = document.getElementById("DataImageName").value;
        var volume_capacity = '<?php echo $_SESSION['volume_capacity']; ?>';
        var volume_size = document.querySelector('input["name=new_volume_size"]').value
        var driver_type = document.querySelector('input["name=new_driver_type"]').value
        var target_bus = document.querySelector('input["name=new_target_bus"]').value
        var storage_pool = document.querySelector('input["name=storage_pool"]').value
        var existing_driver_type = document.querySelector('input["name=existing_driver_type"]').value
        var existing_target_bus = document.querySelector('input["name=existing_target_bus"]').value
        var source_file_cd = document.getElementById("source_file_cd").value;
        var mac_address = document.getElementById("mac_address").value;
        var model_type = document.querySelector('input[name="model_type"]:checked').value;
        var source_network = document.getElementById("source_network").value;
        var xml = '<?php echo $_SESSION['xml'] ?>';
        var dt = new Date(new Date().getTime() - new Date().getTimezoneOffset() * 60 * 1000).toJSON().slice(0, 19).replace(' ', ' ');

        //userid, uuid, action, domName, username, domain_type, domain_name, clock_offset, os_platform, vcpu, memory, memory_unit, source_file_volume, volume_image_name, volume_capacity, volume_size, driver_type, target_bus, storage_pool, existing_driver_type,source_file_cd, mac_address, model_type, source_network, xml_data, dt


        //Fetch POST request
        fetch('https://localhost:3000/api/users/arcs', {
            method: 'POST',
            body: JSON.stringify({
                userid: userid,
                uuid: uuid,
...
<script>
    //logging PHP variables
    console.log(<?php echo json_encode($_SESSION); ?>);
    //$vm
    console.log(<?php echo json_encode($uuid); ?>);
    console.log(<?php echo json_encode($cpu); ?>);
</script>
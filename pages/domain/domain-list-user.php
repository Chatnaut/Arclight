...
if (isset($_POST['action'])) {
  ...
  $_SESSION['t_cores'] = $_POST['tcores'];
  $_SESSION['t_threads'] = $_POST['tthreads'];
...
if ($action == "create-domain") {
  ...
  if ($conn->query($sql) === TRUE && (!$notification)) {
    ...
    if (!$inserttablesql) {
      ...
      // }
      // else{
      echo ("Error description: " . mysqli_error($conn)) . $lv->get_last_error();
    }
  }
...
<script>
  ...
  console.log("<?php echo $_SESSION['domain_name']; ?>");
</script>